...
(Previously: The system MUST isolate the semantic index from the transactional source of truth, enforcing that the semantic index acts strictly as a derived store via an outbox/worker pattern.)
...
(Previously: The system MUST define application ports for reading and writing semantic context to ensure the `Aura.Domain` and `Aura.Application` layers remain decoupled from external SDKs.)
