...
The system MUST play a bundled `.mp3` sound file when a meeting alert arrives. Audio MUST only play after a user gesture to satisfy browser autoplay policy. A silent audio prime MUST be triggered on first user interaction.

#### Scenario: Audio plays after user gesture

- GIVEN the user has interacted with the page at least once
- WHEN a meeting alert arrives
- THEN the alert sound plays via `new Audio("/meeting-alert.mp3").play()`
