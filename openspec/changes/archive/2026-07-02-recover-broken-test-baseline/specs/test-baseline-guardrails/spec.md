# Test Baseline Guardrails Specification

## Purpose

Rules that protect the integration test baseline and E2E selector contracts from silent
breakage during future refactors. Applies to every change touching auth middleware,
JWT wiring, UI markup, or Playwright host configuration.
Decision rule: **regression → fix code; intentional refactor fallout → adapt tests**.

## Requirements

### Requirement: Mock-Auth Integration Contract Preservation
...
### Requirement: E2E Selector Stability Contract

Stable `data-testid` attributes declared in the Playwright E2E spec MUST NOT be
silently removed or renamed. Any change that removes or renames a stable selector MUST
...
- THEN all assertions on selectors listed in `playwright-e2e-bootstrap-spec.md` remain valid
...
### Requirement: Playwright Host Reachability Gate

E2E tests MUST assert that the UI host is reachable before executing any browser-level
assertions. A host unreachability failure MUST surface as a distinct, named test
failure — not as an element-not-found timeout.
...
### Requirement: Refactor-Test Failure Classification
